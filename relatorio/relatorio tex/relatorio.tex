...
\begin{itemize}
  \item \textbf{Davi Nunes Carvalho:} Minerador GitHub (GithubMiner), estruturas de grafos (AbstractGraph, AdjacencyListGraph), exportação Gephi, integração API.
  \item \textbf{Luiz Fernando Batista Moreira:} Análise de grafos (GraphAnalytics), PageRank, betweenness, eigenvector centrality, detecção de comunidades.
  \item \textbf{Josué Carlos Goulart dos Reis:} Testes unitários (84 testes), mock API, validação de API, documentação técnica.
  \item \textbf{João Victor Russo Marquito:} Diagrama UML, relatório LaTeX, definição formal de modelagem, interpretação de resultados.
\end{itemize}
